...
Web は開発中に機能を触るための仮公開だけである。一般向けの本番 Web にはしない。

Flutter Web 本体を公開し続けなくても、アプリは動く。モバイルの Google ログインはネイティブ SDK で、github.io へのリダイレクトは使わない。
github.io の `/AYG/` が必要なのは、Web プレビュー自身の Google ログインだけである。

公開し続ける必要があるのは `legal/` の HTML だけである（App Store が公開 URL を要求するため）。
リリース後は Flutter Web を非公開にしてよい。開発中はプレビューを置いて機能確認してよい。

#### 【Current Implementation】

- Web Preview は GitHub Pages 上で公開されている（開発用）
- Mobile 本線と Web Preview は別 Git ブランチで管理されている
- 公開 HTML の原稿は `legal/`。アプリ内でも同じファイルを全画面表示する
...
| Apple ログイン | Version 1.1 正式スコープに含む | **実装済み**（実機・コンソール確認は未完） |
...
|---|--------|---------------------------|
| 1 | **Apple ログイン** | アプリ実装済み。Services ID Return URL と Supabase Apple プロバイダは Owner 確認 |
| 2 | **カロナビ+（380円 / 4,180円）** | 無料枠と購入画面は実装済み。Store 商品は未作成 |
| 3 | **アカウント削除 RPC** | アプリ実装済み。本番 SQL 未適用 |
| 4 | **App Store 掲載・TestFlight** | 未完了 |
