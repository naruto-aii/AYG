...
Web は開発中に機能を触るための仮公開だけである。アプリをリリースしたら Web は非公開にし、利用者はアプリから使う。一般向けの本番 Web にはしない。

規約・プライバシー・特定商取引法の HTML だけは、App Store が公開 URL を要求するため残す。Flutter Web 本体とは別物である。
